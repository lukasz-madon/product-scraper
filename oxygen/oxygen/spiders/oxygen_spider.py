...
class OxygenSpider(CrawlSpider):
	...
	rules = (
		Rule(SgmlLinkExtractor(allow=(".*aspx$",), deny=(".*GetImage.*")), callback="parse_elem", follow=True),
    )

	def parse_elem(self, response):
		hxs = HtmlXPathSelector(response)
		item = OxygenProduct()			
		item["code"] = response.url[30:-5] #id is URL without the extension and domain	
		
		description = hxs.select('//*[@id="accordion"]/div[1]/text()').extract()	
		item["description"] = description[0].strip().replace("\r\n","") if description else "" 
		
		designer = hxs.select('//*[@id="ctl00_ContentPlaceHolder1_AnchorDesigner"]/text()').extract()
		item["designer"] = designer[0].strip() if designer else ""
		
		price = 0
		sale_discount = 0
		price_before_num = 0
		price1 = self.get_price(hxs.select('//*[@id="container"]/div[2]/span/text()').extract())
		if price1 is not None:
			price = price1
		price2 = self.get_price(hxs.select('//*[@id="container"]/div[3]/span/text()').extract())
		if price2 is not None:
			price = price2
		price_before1 = hxs.select('//*[@id="container"]/div[3]/span/span[1]/span/text()').extract()
		discounted_price1 = hxs.select('//*[@id="container"]/div[3]/span/span[2]/text()').extract()
		...
		price_before2 = hxs.select('//*[@id="container"]/div[2]/span/span[1]/span/text()').extract()
		discounted_price2 = hxs.select('//*[@id="container"]/div[2]/span/span[2]/text()').extract()
		...
		stock_status = {}
		options = hxs.select('//*[@id="ctl00_ContentPlaceHolder1_ddlSize"]/*/text()').extract()
	...
	def get_price(self, elem):
		if len(elem) > 0:			
			price_lst = filter(lambda x: x.isdigit(), elem[0].replace(u'\xa3', ''))
			if len(price_lst) > 0:
	...
	def get_price_and_disc(self, price, disc):
		if len(price) > 0:
			price_before_number = filter(lambda x: x.isdigit(), price[0].replace(u'\xa3', '')) 
			discounted_price_number = filter(lambda x: x.isdigit(), disc[0].replace(u'\xa3', '')) 
